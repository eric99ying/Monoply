...
public class MonoplyView {
    ...
    public MonoplyView(MonoplyController controller){
    	...
    	gamePanel.setBounds(window.getWidth()/2-PANEL_LENGTH/2, window.getHeight()/2-PANEL_LENGTH/2-50, PANEL_LENGTH, PANEL_LENGTH);
    	...
    	playerThreeInfo.setBounds(window.getWidth()-PLAYER_INFO_WIDTH+10, 200, PLAYER_INFO_WIDTH, PLAYER_INFO_LENGTH);
    	
    	playerFourInfo = new JLabel("", SwingConstants.CENTER);
    	playerFourInfo.setBounds(window.getWidth()-PLAYER_INFO_WIDTH+10, 500, PLAYER_INFO_WIDTH, PLAYER_INFO_LENGTH);
    ...
    public void updatePlayerInfo(){
    	playerOneInfo.setText("<html>" + playerOneName + ": $" + controller.model.playerOne.money + "<br>Position: " + controller.players[0].position + "</html>");
    	playerTwoInfo.setText("<html>" + playerTwoName + ": $" + controller.model.playerTwo.money + "<br>Position: " + controller.players[1].position + "</html>");
    	playerThreeInfo.setText("<html>" + playerThreeName + ": $" + controller.model.playerThree.money + "<br>Position: " + controller.players[2].position + "</html>");
    	playerFourInfo.setText("<html>" + playerFourName + ": $" + controller.model.playerFour.money + "<br>Position: " + controller.players[3].position + "</html>");
    ...
    //Updates position information based on which tile the current position is on
    ...
    /*
    //Adds action listeners to the rollDice button
    public void addDiceActionListener(ActionListener listener){
    	rollDice.addActionListener(controller);
    ...
    */
	...
	public void displayDoubleMoveMessageNoJail(Player player, int moves){
		pane.showMessageDialog(window, player.name + " rolled a double and moved " + moves + " steps forward. " + player.name + " will have an extra turn after this turn is over.");
	}
	
	public void displayDoubleMoveMessageJail(Player player){
		pane.showMessageDialog(window, player.name + " rolled three doubles in a row. " + player.name + " was arrested and put in Jail.");
	}
	...
	public int displayPlayerOptions(String[] players,int highestBid, Player bidder){
		return pane.showOptionDialog(window, "<html>Current Highest Bid: "+bidder.name+" $"+highestBid+" <br>Next Bidder:</html>", "Auction", pane.YES_NO_CANCEL_OPTION, pane.QUESTION_MESSAGE, null, players, players[0]);
	}
